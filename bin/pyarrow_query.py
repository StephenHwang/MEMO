...
def load_pq(in_file, query_record, query_start, query_end, k):
    ...
    result = np.apply_along_axis(cast_shadows, 1, genome_omems_arr, k)
...
def merge_intervals(result, num_docs):
    ...
    # concat and sort
    fully_merged_sorted_array = np.concatenate(merged_intervals)
    return fully_merged_sorted_array[np.lexsort((fully_merged_sorted_array[:,1], fully_merged_sorted_array[:,0]))]
