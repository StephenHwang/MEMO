#!/usr/bin/env python3
#
# Run: Scratch filter_pq functions for extracting bed-intervals overlapping query region
#  ./parquet_extract_bed_scratch.py \
#    -b bed.parquet \
#    -r chr:start-end \
#    -o out.bed
...
def filter_pq(in_file, query_record, query_start, query_end):
    ...
    This one is one ended up using.
    '''
...
def filter_pq_simple(in_file, query_record, query_start, query_end):
    ...
    This one is too simple, only extracts bed-intervals that are fully contained in the query region.
    '''
    e_coli_mem_bed_pq_ds = ds.dataset(in_file, format="parquet")
    interval_filter_1 = (
        (ds.field('f0') == query_record) &
    ...
    return genome_omems_arr_1


def filter_pq_old(in_file, query_record, query_start, query_end):
    '''
    Filter parquet bed file into k-mer shadow casted intervals.

    This one is more complicated than necessary, but should be correct.
    '''
    e_coli_mem_bed_pq_ds = ds.dataset(in_file, format="parquet")
    interval_filter_1 = (
        (ds.field('f0') == query_record) &
